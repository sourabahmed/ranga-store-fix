...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product">
      ...
      <h4 class="text-danger">Rating:${rating.rate}(${rating.count})</h4>
...
const setInnerText = (id, value) => {
  const a = document.getElementById(id).innerText = value.toFixed(2);
...
const updateTotal = () => {
  const grandTotal =
   ...
   const b = document.getElementById("total").innerText = grandTotal.toFixed(2);
   //console.log(b)
};
loadProducts();
